...
class RadosScrubber:
    ...
    def run_container_cmd(self, cmd: str):
        """
        Runs the command on conatiner and returns
        the output
        Args:
    ...
    def get_cluster_date(self):
        ...
        out = self.run_container_cmd(cmd=cmd)
        out = out.strip()
        return out
    ...
    def get_pg_dump(self, *args):
        ...
        pgDump = self.rados_obj.run_ceph_command(cmd=cmd)
        if args:
            columns = list(args)
