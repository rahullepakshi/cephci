...
def run(ceph_cluster, **kw):
    ...
    3. ceph fs subvolume create <vol_name> <subvol_name> [--size <size_in_bytes>] [--group_name <subvol_group_name>]
       ...
       Ex: ceph fs subvolume create cephfs subvol_clone_states --size 5368706371 --group_name subvolgroup_
    ...
    5. Create snapshot of the subvolume
        Ex: ceph fs subvolume snapshot create cephfs subvol_clone_states snap_1 --group_name subvolgroup_1

    Clone Operations and Clone States:
    1. Create a clone in default locaction.
        ceph fs subvolume snapshot clone cephfs subvol_clone_states snap_1 clone_status_1 --group_name subvolgroup_1
    ...
    try:
        ...
        subvolumegroup_list = [
            {"vol_name": default_fs, "group_name": "subvolgroup_clone_states_1"}
        ]
        for subvolumegroup in subvolumegroup_list:
            fs_util.create_subvolumegroup(client1, **subvolumegroup)
        subvolume = {
            "vol_name": default_fs,
            "subvol_name": "subvol_clone_states",
            "group_name": "subvolgroup_clone_states_1",
            "size": "5368706371",
        }
        ...
        subvol_path, rc = client1.exec_command(
            sudo=True,
            cmd=f"ceph fs subvolume getpath {default_fs} subvol_clone_states subvolgroup_clone_states_1",
        )
